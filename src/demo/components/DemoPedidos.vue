<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center space-x-6">
          <div class="text-center">
            <div class="text-lg font-semibold text-orange-600">{{ demoStore.estadoPedidos.pendientes }}</div>
            <div class="text-xs text-gray-500">Pendientes</div>
          </div>
          <div class="text-center">
            <div class="text-lg font-semibold text-blue-600">{{ demoStore.estadoPedidos.en_preparacion }}</div>
            <div class="text-xs text-gray-500">En preparación</div>
          </div>
          <div class="text-center">
            <div class="text-lg font-semibold text-green-600">{{ demoStore.estadoPedidos.listos }}</div>
    ...
    <div class="space-y-4">
      <div
        ...
        :class="[
          ...
          demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) > 60 && 'border-red-300 bg-red-50/50',
          demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) > 30 && demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) <= 60 && 'border-yellow-300 bg-yellow-50/50'
        ]"
      >
        <!-- Header del pedido -->
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-6">
              ...
              <div class="text-right">
                <div :class="[
                  'text-sm font-medium',
                  demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) > 60 ? 'text-red-600' :
                  demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) > 30 ? 'text-yellow-600' :
                  'text-green-600'
                ]">
                  {{ demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) }} min
                </div>
        ...
        <div class="px-6 py-3 bg-gray-50/50 border-t border-gray-200/50 rounded-b-2xl">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4 text-sm text-gray-500">
              ...
              <span>{{ pedido.comensales }} comensales</span>
              <span v-if="demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) > 30" class="text-red-600 font-medium">
                ⚠️ {{ demoStore.calcularTiempoTranscurrido(pedido.hora_pedido) > 60 ? 'Muy atrasado' : 'Atrasado' }}
              </span>
            </div>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- Items de Cocina -->
      <div class="demo-card">
        <div class="bg-red-500 px-4 py-3 rounded-t-2xl">
        ...
        <div class="p-4 space-y-3 max-h-96 overflow-y-auto">
          <div
            ...
            :class="[
              ...
              item.tiempo_transcurrido > 20 && 'border-red-300 bg-red-50',
              item.tiempo_transcurrido > 10 && item.tiempo_transcurrido <= 20 && 'border-yellow-300 bg-yellow-50'
            ]"
          ...
          >
            <div class="flex justify-between items-start mb-2">
              <div>
                ...
                <div class="text-xs text-gray-500">Mesa {{ item.numero_mesa }}</div>
              </div>
              <div class="text-right">
                <div :class="[
                  'text-sm font-medium',
                  item.tiempo_transcurrido > 20 ? 'text-red-600' :
                  item.tiempo_transcurrido > 10 ? 'text-yellow-600' :
                  'text-green-600'
                ]">
          ...
          <div v-if="itemsCocina.length === 0" class="text-center py-8 text-gray-500 text-sm">
            🎉 ¡Excelente! No hay items pendientes en cocina
          </div>
        </div>
      </div>
      ...
      <div class="demo-card">
        ...
        <div class="p-4 space-y-3 max-h-96 overflow-y-auto">
          ...
          <div v-if="itemsBarra.length === 0" class="text-center py-8 text-gray-500 text-sm">
            🎉 ¡Perfecto! No hay items pendientes en barra
          </div>
        </div>
      </div>
    </div>
...
const pedidosFiltrados = computed(() => {
  return demoStore.pedidosActivos.filter(p => filtroEstado.value.includes(p.estado))
})

const itemsCocina = computed(() => 
  demoStore.itemsPendientes.filter(i => !i.estacion_preparacion || i.estacion_preparacion === 'cocina')
)

const itemsBarra = computed(() => 
  demoStore.itemsPendientes.filter(i => i.estacion_preparacion === 'barra')
)
...
const contarPorEstado = (estado: string) => {
  return demoStore.pedidosActivos.filter(p => p.estado === estado).length
}
...
const cambiarEstadoPedido = (pedidoId: string, nuevoEstado: string) => {
  demoStore.actualizarEstadoPedido(pedidoId, nuevoEstado)
...
const cambiarEstadoItem = (item: any) => {
  ...
  if (nuevoEstado) {
    demoStore.actualizarEstadoItem(item.id, nuevoEstado)
...
const verDetallesPedido = (pedido: any) => {
  const detalles = `
🍽️ DETALLES DEL PEDIDO ${pedido.numero_pedido}

📍 Mesa: ${pedido.numero_mesa}
👤 Cliente: ${pedido.cliente_nombre}
⏰ Tiempo: ${demoStore.calcularTiempoTranscurrido(pedido.hora_pedido)} minutos
...
${pedido.notas_cocina ? `👨‍🍳 Cocina: ${pedido.notas_cocina}` : ''}

¡En la versión completa puedes gestionar cada detalle!
  `
  alert(detalles)
}
...
const simularNuevoPedido = () => {
  ...
  if (mesasLibres.length === 0) {
    mostrarNotificacion('No hay mesas libres para simular un nuevo pedido', 'warning')
    return
  }
  
  const mesaAleatoria = mesasLibres[Math.floor(Math.random() * mesasLibres.length)]
  const clientesDemo = ['Juan Pérez', 'Laura Sánchez', 'Miguel Torres', 'Carmen López', 'David Ruiz', 'Patricia Moreno']
  ...
  mostrarNotificacion(`¡Nuevo pedido simulado! ${nuevoPedido.numero_pedido} en Mesa ${nuevoPedido.numero_mesa}`, 'success')
}
